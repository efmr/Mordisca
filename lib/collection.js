...
function set(self, redisClient, docs, mode, options, callback) {
  ...
  async['each' + method](objs,
    function(doc, seqCallback) {
      ...
      if (mode === 'cache' || mode === 'snapshot') {
        redisClient.lpush(self.redisPrefix + '_cache',
          self.redisPrefix + hash);
        if(mode === 'snapshot') {
          redisClient.hmset(
            self.redisPrefix + '_snapshot:' + (doc._snapshot || 'snapshot'),
            u.reddify(doc, hash)
          );
        }
      }
...
function snapshot(self, docs, options, callback) {
  callback = callback || options;
  var opts = callback !== options ? _.defaults(options, dOpts) : dOpts,
    objs = Array.isArray(docs) ? docs : [docs],
  ...
  set(self, redisClient, mObjs, 'snapshot', opts, function(err) {
    if(err) {
      if(typeof callback === 'function') {
        return callback(err);
      }
    ...
    function(err, hash) {
      ...
      self.cacheSaver.hgetall(hash, function(err, obj) {
        ...
        self.dbCollection.insert(doc, opts, function(err) {
          ...
          self.cacheSaver.lrem(self.redisPrefix + '_save', -1,
            hash, cb);
        });
      });
    }
...
function Collection(cacheLayer, col, options) {
  ...
  self.cacheListener.on('message', function(ch, msg) {
    ...
    if (/__keyspace@0__:/.test(ch)) {
      ...
      if(syncing[index] || index === '_snapshot'){
        return;
      }
      syncing[index] = true;
      setTimeout(function() {
...
Collection.prototype.snapshot = function(docs, options, callback) {
  snapshot(this, docs, options, callback);
};
