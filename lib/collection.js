...
function set(self, redisClient, docs, mode, options, callback) {
  ...
  async['each' + method](objs,
    function(doc, seqCallback) {
      ...
      if (mode === 'cache') {
        redisClient.lpush(self.redisPrefix + 'cache',
          self.redisPrefix + hash);
      }
...
function saveCache(self, options, callback) {
  ...
  self.cacheSaver.brpoplpush(self.redisPrefix + 'cache',
    self.redisPrefix + 'save',
    10,
    function(err, hash) {
      if (err || hash === null) {
        return cb(err, hash);
      ...
      self.cacheSaver.hgetall(hash, function(err, obj) {
        ...
        self.dbCollection.insert(doc, opts, function(err) {
          ...
          self.cacheSaver.lrem(self.redisPrefix + 'save', -1, hash, cb);
        });
      });
    }
  );
...
function Collection(cacheLayer, col) {
  ...
  self.cacheListener.subscribe('__keyspace@0__:' + self.redisPrefix + 'cache');
  self.cacheListener.on('message', function(ch, msg) {
    ...
    if (ch === '__keyspace@0__:' + self.redisPrefix + 'cache') {
      if (saving) {
        return;
      }
      saving = true;
